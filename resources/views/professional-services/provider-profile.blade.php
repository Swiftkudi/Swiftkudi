            ...
            <div class="space-y-6 !pt-0 !min-h-0">
                <section class="marketplace-card p-6 sm:p-8">
                    <div class="flex flex-col gap-5 sm:flex-row sm:items-start">
                        ...
                        <div class="min-w-0 flex-1"><div class="flex flex-wrap items-center gap-2"><h1 class="font-heading text-2xl font-bold text-white">{{ $profile->user->name }}</h1>@if($profile->user->marketplace_seller_verified)<span class="marketplace-status marketplace-status-info"><i class="fas fa-circle-check mr-1"></i>Verified</span>@endif @if($profile->is_available)<span class="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-300">Available</span>@endif</div><p class="mt-1.5 text-lg text-gray-300">{{ $profile->professional_title ?: 'Independent professional' }}</p><div class="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-gray-400">@if($profile->total_reviews > 0)<span><i class="fas fa-star mr-1 text-amber-400"></i><strong class="text-gray-200">{{ number_format((float)$profile->average_rating, 1) }}</strong> · {{ $profile->total_reviews }} review{{ $profile->total_reviews === 1 ? '' : 's' }}</span>@else<span>New talent · no reviews yet</span>@endif<span><i class="fas fa-check-circle mr-1 text-emerald-400"></i>{{ $profile->total_orders_completed }} completed order{{ $profile->total_orders_completed === 1 ? '' : 's' }}</span>@if($profile->hourly_rate)<span><strong class="text-white">₦{{ number_format((float)$profile->hourly_rate) }}</strong>/hr</span>@endif</div>@if($profile->availability_note)<p class="mt-3 text-sm text-gray-500">{{ $profile->availability_note }}</p>@endif</div>
                    </div>
                ...
                @if($completedContracts->isNotEmpty())
                    <section class="marketplace-card overflow-hidden">
                        <div class="border-b border-dark-700 px-5 py-4 sm:px-6"><h2 class="text-lg font-semibold text-white">Work history</h2><p class="mt-1 text-sm text-gray-500">Completed freelance contracts recorded on SwiftKudi.</p></div>
                        @foreach($completedContracts as $contract)
                            <div class="marketplace-list-row">
                                <span class="marketplace-icon-box flex-none"><i class="fas fa-briefcase"></i></span>
                                <div class="min-w-0 flex-1"><div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between"><h3 class="font-semibold text-white">{{ $contract->title }}</h3><span class="font-semibold text-gray-300">₦{{ number_format((float)$contract->amount, 0) }}</span></div><p class="mt-1 text-sm text-gray-500">Completed {{ optional($contract->completed_at)->format('M Y') ?: optional($contract->updated_at)->format('M Y') }}@if($contract->client) · Client: {{ $contract->client->name }}@endif</p></div>
                            </div>
                        @endforeach
                    </section>
                @endif
                ...
                @if($reviews->isNotEmpty())
                    <section class="marketplace-card overflow-hidden">
                        <div class="border-b border-dark-700 px-5 py-4 sm:px-6"><h2 class="text-lg font-semibold text-white">Client feedback</h2><p class="mt-1 text-sm text-gray-500">Reviews tied to completed professional-service orders.</p></div>
                        @foreach($reviews as $review)
                            <article class="marketplace-list-row">
                                ...
                                <div class="min-w-0 flex-1"><div class="flex flex-wrap items-center justify-between gap-2"><div><p class="font-semibold text-white">{{ optional($review->order?->service)->title ?: 'Professional service' }}</p><p class="mt-1 text-xs text-gray-600">{{ optional($review->reviewer)->name ?: 'Client' }} · {{ optional($review->created_at)->format('M Y') }}</p></div><div class="whitespace-nowrap text-sm font-semibold text-gray-200"><i class="fas fa-star mr-1 text-amber-400"></i>{{ (int)$review->rating }}/5</div></div><p class="mt-3 whitespace-pre-line text-sm leading-6 text-gray-400">{{ $review->comment }}</p>@if($review->response)<div class="mt-3 rounded-lg border border-dark-700 bg-dark-950 p-3 text-sm text-gray-500"><strong class="text-gray-300">Freelancer response:</strong> {{ $review->response }}</div>@endif</div>
                            </article>
                        @endforeach
                    </section>
                @endif
