...
@section('title', $growthListing->title . ' - Growth Marketplace')
                ...
                <div class="bg-dark-900 rounded-2xl shadow-lg border border-dark-700 p-4 lg:p-6">
                    <span class="px-3 py-1 text-xs rounded-full mb-4 inline-block font-medium
                        ...
                        <i class="fas 
                            ...
                            @endswitch mr-1"></i>
                        {{ ucfirst($growthListing->type) }}
                    </span>

                    <h1 class="text-xl lg:text-2xl font-bold text-white mb-4">{{ $growthListing->title }}</h1>
                    <p class="text-gray-400 mb-6 text-sm lg:text-base">{{ $growthListing->description }}</p>

                    @if($growthListing->specs)
                        @php
                            $specs = is_string($growthListing->specs)
                                ? json_decode($growthListing->specs, true)
                                : $growthListing->specs;
                        @endphp
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-medium text-white">
                                    {{ $growthListing->seller->name ?? 'Unknown' }}
                                </p>

                                <p class="text-sm text-gray-400">
                                    Member since {{ $growthListing->seller && $growthListing->seller->created_at ? $growthListing->seller->created_at->format('M Y') : 'N/A' }}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div>
                <div class="bg-dark-900 rounded-2xl shadow-lg border border-dark-700 p-4 lg:p-6 sticky top-20">
                    <div class="text-center mb-6">
                        <div class="text-3xl lg:text-4xl font-bold text-green-400 mb-1">
                            ₦{{ number_format($growthListing->price, 2) }}
                        </div>

                        <div class="flex items-center justify-center gap-2 text-sm text-gray-400">
                            <i class="fas fa-clock"></i>
                            <span>{{ $growthListing->delivery_days }} days delivery</span>
                        </div>
                    </div>
                    ...
                    @auth
                        ...
                        @if(auth()->id() !== $growthListing->user_id)
                            <a href="{{ route('chat.open', ['type' => 'growth_service', 'referenceId' => $growthListing->id, 'participantId' => $growthListing->user_id]) }}" class="mt-3 w-full py-3 border-2 border-purple-500/30 text-purple-400 font-medium rounded-xl flex items-center justify-center gap-2 hover:bg-purple-500/10 transition-colors">
    ...
    <div class="bg-dark-900 rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-dark-700">
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 rounded-t-3xl">
            ...
            <p class="text-indigo-200 mt-1">
                Send a message to {{ $growthListing->seller->name ?? 'the seller' }}
            </p>
        </div>
        ...
        <form id="contact-form" class="p-6 space-y-6">
            ...
            <input type="hidden" name="recipient_id" value="{{ $growthListing->user_id }}">
            <input type="hidden" name="listing_id" value="{{ $growthListing->id }}">
...
<div
    ...
    data-order-url="{{ route('growth.order', $growthListing->id) }}"
